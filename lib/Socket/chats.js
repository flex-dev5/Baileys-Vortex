...
const call = async (jid, type, offer) => {
    if (!['audio', 'video'].includes(type)) {
        throw new boom_1.Boom('Invalid call type. Must be "audio" or "video"');
    }

    const callId = generateMessageTag() + '@' + WABinary_1.S_WHATSAPP_NET;
    ...
    const callNode = {
        ...
        attrs: {
            ...
            id: callId,
            type
            // تمت إزالة chatid تمامًا
        },
        content: offer ? [
            {
                tag: 'offer',
                attrs: {},
                content: WAProto_1.proto.CallOffer.encode(offer).finish()
            }
        ] : undefined
    };

    await sendNode(callNode);
    return callId; // إرجاع callId فقط
};

// معالج تحديثات المكالمات (بدون chatid)
ws.on('CB:call', (node) => {
    ...
    switch (type) {
        case 'offer':
            const offerNode = WABinary_1.getBinaryNodeChild(node, 'offer');
            if (offerNode?.content) {
                updateData.offer = WAProto_1.proto.CallOffer.decode(offerNode.content);
            }
            break;
        ...
        case 'accept':
            case 'reject':
            case 'terminate':
                const contentNode = WABinary_1.getBinaryNodeChild(node, type);
                if (contentNode?.content) {
                    updateData[type] = WAProto_1.proto[`Call${type.charAt(0).toUpperCase() + type.slice(1)}`].decode(contentNode.content);
                }
                break;

        case 'ring':
            updateData.isRinging = true;
            break;
    }

    ev.emit('call.update', updateData);
});

// دالة إنهاء المكالمة (بدون chatid)
const endCall = async (callId, jid, reason) => {
    const from = authState.creds.me.id;
    ...
    const callNode = {
        ...
        content: reason ? [
            {
                tag: 'terminate',
                attrs: { reason: reason.reason || 'normal' },
                content: WAProto_1.proto.CallTerminate.encode(reason).finish()
            }
        ] : undefined
    };
...
// دالة قبول المكالمة (بدون chatid)
const acceptCall = async (callId, jid, accept) => {
    const from = authState.creds.me.id;
        ...
        content: [
            {
                tag: 'accept',
                attrs: {},
                content: WAProto_1.proto.CallAccept.encode(accept).finish()
            }
        ]
    };
...
// دالة رفض المكالمة (بدون chatid)
const rejectCall = async (callId, jid, reason) => {
    const from = authState.creds.me.id;
    ...
    const callNode = {
        ...
        content: [
            {
                tag: 'reject',
                attrs: { reason: reason.reason || 'busy' },
                content: WAProto_1.proto.CallReject.encode(reason).finish()
            }
        ]
    };
