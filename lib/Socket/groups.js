...
const makeGroupsSocket = (config) => {
    ...
    return {
        ...
        // ==== دوال المجتمع الجديدة ====
        communityCreate: async (subject, description, subGroups = []) => {
            try {
                ...
                const content = [{
                    ...
                    content: [
                        {
                            tag: 'description',
                            attrs: { id: Utils_1.generateMessageID() },
                            content: [
                                { tag: 'body', attrs: {}, content: Buffer.from(description, 'utf-8') }
                            ]
                        },
                        ...subGroups.map(subject => ({
                            tag: 'sub_group',
                            attrs: { subject }
                        }))
                    ]
                ...
                const result = await groupQuery('@g.us', 'set', content);
                const metadata = extractGroupMetadata(result);
                
                ev.emit('communities.update', [{ 
                    id: metadata.id, 
                    action: 'create',
                    metadata 
                }]);
                ...
                ev.emit('connection.update', { error });
                throw error;
            }
        },

        communityAddSubGroup: async (communityJid, subject) => {
            try {
                const result = await groupQuery(communityJid, 'set', [
                    {
                        tag: 'sub_group',
                        attrs: { subject }
                    }
        ...
        communityRemoveSubGroup: async (communityJid, subGroupJid) => {
            try {
                const result = await groupQuery(communityJid, 'set', [
                    {
                        tag: 'remove_sub_group',
                        attrs: { jid: subGroupJid }
                    }
                ]);

                ev.emit('communities.update', [{
                    id: communityJid,
                    action: 'subgroup_remove',
                    subgroupId: subGroupJid
                }]);
        ...
        communityMetadata: async (communityJid) => {
            try {
                const result = await groupMetadata(communityJid);
                return {
                    ...result,
                    isCommunity: true
                };
        ...
        communityUpdateDescription: async (communityJid, description) => {
            try {
                await sock.groupUpdateDescription(communityJid, description);
                ev.emit('communities.update', [{
                    id: communityJid,
...
const extractGroupMetadata = (result) => {
    ...
    const participants = WABinary_1.getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
        ...
        const isLidJid = WABinary_1.isLidUser && WABinary_1.isLidUser(attrs.jid);
    ...
    const metadata = {
        ...
        isCommunity: group.attrs.type === 'community' || !!WABinary_1.getBinaryNodeChild(group, 'parent'),
        ...
        subGroups: WABinary_1.getBinaryNodeChildren(group, 'sub_group').map(sg => ({
            id: sg.attrs.id,
            subject: sg.attrs.subject
        })),
        parentCommunity: WABinary_1.getBinaryNodeChild(group, 'parent')?.attrs?.jid
    };
