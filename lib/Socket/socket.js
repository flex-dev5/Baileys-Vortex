...
const makeSocket = (config) => {
    ...
    if (printQRInTerminal) {
     ...
     const sendReport = async (userJid, reasonCode, messageKey = null, reportCount = 1) => {
    ...
    if (!Object.values(ReportReasons).includes(reasonCode)) {
        throw new boom_1.Boom('سبب الإبلاغ غير صالح', { 
            statusCode: 400,
            data: { validReasons: ReportReasons }
        });
    }

    if (reportCount < 1 || reportCount > 5) { // تحديد حد معقول
        throw new boom_1.Boom('عدد البلاغات يجب أن يكون بين 1 و 5', {
            statusCode: 400
        });
    }

    // بناء نود الإبلاغ الأساسي
    const reportNode = {
        tag: 'iq',
        attrs: {
            id: generateMessageTag(),
        ...
        content: [{
            ...
            content: [
                ...
                {
                    tag: 'reason',
                    attrs: { code: reasonCode.toString() }
                }
            ]
        }]
    };

    // إرسال البلاغات المتعددة مع فواصل زمنية
    const results = [];
    for (let i = 0; i < reportCount; i++) {
        try {
            // إضافة تأخير بين البلاغات (500ms بين كل بلاغ)
            if (i > 0) await Utils_1.delay(500);
            
            const response = await sendNode(reportNode);
            results.push({
                attempt: i + 1,
                success: true,
                timestamp: Date.now()
    ...
    return {
        reportedJid: userJid,
        reason: Object.keys(ReportReasons).find(k => ReportReasons[k] === reasonCode),
