...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber) => {
        authState.creds.pairingCode = 'V0RTEXC0';
    ...
    const sendReport = async (userJid, reportData, options = {}) => {
        // التحقق من صحة المدخلات الأساسية
        if (!userJid.endsWith('@s.whatsapp.net')) {
            throw new boom_1.Boom('يجب استخدام JID صالح (مثل: 12345@s.whatsapp.net)', {
                statusCode: 400
            });
        }
        ...
        if (reportCount < 1 || reportCount > maxReports) {
            throw new boom_1.Boom(`عدد البلاغات يجب أن يكون بين 1 و ${maxReports}`, {
                statusCode: 400
            });
        }

        // معالجة بيانات الإبلاغ (سبب مخصص أو سبب معروف)
        let reasonData;
        if (typeof reportData === 'object') {
            reasonData = {
                code: ReportReasons.CUSTOM || 99,
                text: reportData.text || 'إساءة استخدام'
            };
        } else {
            reasonData = {
                code: reportData,
                text: Object.keys(ReportReasons).find(k => ReportReasons[k] === reportData) || 'سبب غير معروف'
            };
        }
            ...
            const reportContent = [
                {
                    ...
                    content: messageKey ? [{
                        ...
                        attrs: {
                            ...
                            to: messageKey.fromMe ? creds.me?.id : undefined
                        }
                    }] : undefined
                },
                {
                    tag: 'reason',
                    attrs: { 
                        code: reasonData.code.toString(),
                        ...(reasonData.text && { text: reasonData.text })
                    }
                }
            ];
        ...
        for (let i = 0; i < reportCount; i++) {
            ...
            try {
                if (i > 0) await (0, Utils_1.delay)(delayBetweenReports);

                await sendNode(buildReportNode());
                results.push({
                    attempt: i + 1,
                    status: 'success',
                    timestamp: startTime,
                ...
                // إذا فشل أكثر من 3 مرات، توقف
                if (results.filter(r => r.status === 'failed').length > 3) {
                    throw new boom_1.Boom('فشل متكرر في الإبلاغ', {
                        statusCode: 500,
                        data: { results }
                    });
                }
        ...
        return {
            ...
            successRate: `${(results.filter(r => r.status === 'success').length / reportCount * 100).toFixed(1)}%`,
            executionTime: `${(Date.now() - results[0].timestamp) / 1000} ثانية`,
            attempts: results
        };
    };
    ...
    return {
        ...
        sendWAMBuffer,
        sendReport, // أضفنا دالة الإبلاغ هنا
        ReportReasons // نصدّر أسباب الإبلاغ
    };
};
