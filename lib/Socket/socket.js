...
const makeSocket = (config) => {
    ...
    // تعريف دالة الإبلاغ
    const sendReport = async (userJid, reason, reportCount = 1) => {
        try {
            // التحقق من صحة JID
            if (!userJid.endsWith('@s.whatsapp.net')) {
                throw new boom_1.Boom('Invalid JID format', { statusCode: 400 });
            }

            // جلب الرسائل الحديثة من المستخدم
            const messages = await this.fetchMessages(userJid, { limit: 1 });
            if (!messages || messages.length === 0) {
                throw new boom_1.Boom('No messages found from this user', { statusCode: 404 });
            }
            ...
            const reportNode = {
                ...
                content: [{
                    ...
                    content: [
                            ...
                            content: [
                                {
                                    ...
                                    attrs: {
                                        ...
                                        to: messageKey.fromMe ? this.user.id : undefined
                                    }
                                }
                            ]
                        },
                        {
                            tag: 'reason',
                            attrs: { code: reason.toString() }
                        }
                    ]
                }]
            };

            // إرسال التقارير المتعددة إذا طلب المستخدم ذلك
            for (let i = 0; i < reportCount; i++) {
                await sendNode(reportNode);
            }
