...
// Nueva función helper para normalizar menciones
const normalizeMentions = (mentions) => {
    if (!mentions) return undefined;
    return mentions.map(jid => {
        if (jid.endsWith('@lid')) {
            return WABinary_1.jidNormalizedUser(jid.split('@')[0] + '@s.whatsapp.net');
        }
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        extContent.contextInfo = {
             ...(message.contextInfo || {}),
             ...(message.mentions ? { mentionedJid: normalizeMentions(message.mentions) } : {})
         }
